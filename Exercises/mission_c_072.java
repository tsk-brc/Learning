...
public class Main {
    public static void main(String[] args) {
        ...
        int data = 0;
        for(int k = 0; k < num; k++){
          if(atk >= Integer.parseInt(str[k][1]) && atk <= Integer.parseInt(str[k][2]) && 
             def >= Integer.parseInt(str[k][3]) && def <= Integer.parseInt(str[k][4]) &&
             agi >= Integer.parseInt(str[k][5]) && agi <= Integer.parseInt(str[k][6])){
             name[data] = str[k][0];
             data++;
          }
        }
