...
config.read(os.getcwd() + '/expectation/config.ini', encoding='utf-8')
...
def get_race_html_by_year_and_month(year, month):
    # 対象年のファイルを開く
    with open(RACE_URL_DIR + str(year) + str('{0:02d}'.format(month)) + ".txt", "r") as f:
        ...
        if len(urls) != len(file_list):
            ...
            # 処理結果を出力
            logging.info(str(
                year) + "年" + str('{0:02d}'.format(month)) + "月のHTMLを" + str(len(urls)) + "件保存しました")
        else:
            # 全てのデータを取得済の場合はログ出力のみ行う
            logging.info(str(year) + "年" +
                         str('{0:02d}'.format(month)) + "月のHTMLは取得済のためスキップします")
...
if __name__ == '__main__':
    ...
    # ログファイルを定義
    logging.basicConfig(filename='expectation/log/activity.log',
