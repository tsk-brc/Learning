...
class MissionC080
  def run
    ...
    game_rule = $stdin.gets.split.map!(&:to_i)
    # ボタンを押した回数
    ...
    score = 0
    # 押したボタン
    push_button = $stdin.gets.split.map!(&:to_i)
    # ボタンを押した回数分ループ
    push_button_count.times do |num|
      # 押したボタンの正誤判定(正解なら+1000点、不正解なら不正解数を1プラスする)
      if push_button[num] == should_push_button_number
        score += 1000
      else
        wrong_count += 1
      end
      # 不正解数がゲームオーバーの回数に到達したらループを抜ける
      break if wrong_count == game_rule[1]
      # 次に正解になるボタン番号を更新
      should_push_button_number == game_rule[0] ?
      should_push_button_number = 1 : should_push_button_number += 1
    end
    # スコアを出力
    puts wrong_count != game_rule[1] ? score : -1
  end
end
