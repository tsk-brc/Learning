...
class MissionB076
  def run
    ...
    bakery_info = $stdin.gets.split.map!(&:to_i)
    # パンの価格を格納する配列
    bread_price = []
    # パンの残数を格納する配列
    bread_number = []
    # パンの種類数分ループ
    bakery_info[0].times { |num|
      # パンの情報(パンの価格 パンの残数)
      bread_info = $stdin.gets.split.map!(&:to_i)
      # パンの価格と残数をそれぞれの配列に格納
      bread_price[num] = bread_info[0]
      bread_number[num] = bread_info[1]
    }
    # パンの操作数分ループ
    bakery_info[1].times { |num|
      # 購入可能かどうかを表すフラグ
